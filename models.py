...
class QuoteManager(models.Manager):
	def createitem(self, adduser, item, quoted_by):
		errors = {}

		if len(item) < 10:
			errors['item'] = "Item name must be longer than 10 characters"
		if item == "":
		...
		if errors:
			...
			self.create(adduser = adduser, item = item, quoted_by = quoted_by)
			return (True, "true")

	def joinlist(self, joindude, itemid):
		try:
			the_item = Quote.QuoteManager.get(id = itemid)
	...
	def removeitem(self, remover, itemid):
		try:
			remove_item = Quote.QuoteManager.get(id = itemid)
			test = remove_item.joinuser.remove(remover)
			print remove_item.joinuser.all()
			result = {"error": "Successfully remove item from your Quote"}
			return (True, result )
		except:
	...
	def deleteitem(self, adduser, itemid):
		try:
			del_item = Quote.QuoteManager.get(adduser = adduser, id = itemid).delete()
			return (True, 'true')
		except:
			return (False, 'false')
...
class Quote(models.Model):
	...
	quoted_by = models.CharField(max_length = 45, blank = True)
	created_at = models.DateTimeField(auto_now_add = True)
	updated_at = models.DateTimeField(auto_now = True)
